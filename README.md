...
当前版本为 `1.1.0`，构建配置支持 Minecraft `1.21.1`、`1.21.4`、`1.21.8`、`1.21.10`、`1.21.11`、`26.1.2` 和 `26.2`。
...
- 截图复制到剪贴板、鞘翅飞行时间、资源包检查跳过、服务器图标缓存和地图物品预览。
- 船只 360°视角、划船手持物显示、背包移动、冷却自动攻击、列表拖动和快捷键圆盘。
- 游戏内输入法、点击发送、快速加载、提示消息屏蔽和服务器列表刷新修复。
- Tweakeroo 自由相机容器交互、使用物品、挖掘和 Litematica Easy Place 联动。
- 配置说明按当前屏幕宽度自动换行，配置页与 Mod Menu 分别记忆滚动位置。

## 重要说明

Fabric Loader `0.16.14+` 和 MaLiLib 是必需依赖。Xaero's Minimap、Xaero's World Map、Mod Menu、Tweakeroo、Carpet、REI 和 JEI 为可选联动模组，不会打进 halfmasa JAR；未安装对应模组时，相关兼容功能不会启用。

配置保存在 `config/halfmasa/`。搜索与查询历史、快捷键圆盘数据和服务器图标缓存使用该目录下的独立文件或子目录。模组是纯客户端工具，不要求服务器安装 halfmasa。
...
项目主体按 [MIT License](LICENSE) 发布。部分功能参考或移植自 TechUtils、JEI Recipe History、REI、InvMove、BoatView360、Boat Item View、ElytraTime 等项目；对应许可证和归属说明见源码及 JAR 内的 `META-INF/halfmasa/THIRD_PARTY_NOTICES.md`。
