...
| 配置或动作 | 默认值 | 说明 |
|---|---|---|
| `enableWorldBinding` | `false` | 在单人存档根目录保存 Xaero Minimap 与 World Map 的根目录 ID，使改名、移动和备份恢复后的存档继续使用同一路径点与地图数据。 |
| `customSavesPaths` | 空列表 | 按顺序选择第一个有效的自定义 `saves` 路径；支持绝对路径和相对游戏目录的路径，修改后重启生效。 |
...
历史栏会在物品管理器首次打开时初始化，不需要先搜索配方。查询历史保存在 `config/halfmasa/search-history/`，JEI 与 REI 使用独立文件。
...
| `serverIconCache` | `false` | 缓存服务器图标，可按名称、地址或两者匹配，并设置缓存数量上限。 |
...
快捷键圆盘数据位于 `config/halfmasa/keybind-pie/bindings.json`；服务器图标缓存位于 `config/halfmasa/server-icons/`。

## Tweakeroo 自由相机扩展

安装 Tweakeroo 后，可以分别启用自由相机下的容器交互、物品使用、方块挖掘和 Litematica Easy Place。各子功能默认关闭，且只在对应模组和功能可用时生效。
...
主配置文件为 `config/halfmasa/halfmasa.json`。旧版根目录配置会迁移到该目录，旧文件会保存到 `config/halfmasa/legacy/`。功能数据使用临时文件和原子替换写入，避免正常保存过程中留下不完整 JSON。
